...
public class LocalDateTp {
	...
	public static void main(String[] args) {
		...
		int year2000 = 2000;
		System.out.println("\r\nAffichage de tous les mois de l'année : %1$s".formatted(year2000));
		for (int month = 1; month <= 12; month++) {
			YearMonth yearMonth = YearMonth.of(year2000, month);
			System.out.println("La durée de %1$s est de %2$s jours".formatted(yearMonth, yearMonth.lengthOfMonth()));
		}
		//Cf. code Mathilda
		//TODO : trouver pourquoi nombre de jours == 0
		LocalDate year = LocalDate.now();
		int i = 0;
		while (i < 12) {
			year.plusMonths(1);
			LocalDate moisEnCours = year.plusMonths(i);
			System.out.println(moisEnCours.getMonth());
			long days = ChronoUnit.DAYS.between(moisEnCours.with(TemporalAdjusters.firstDayOfMonth()),
					moisEnCours.with(TemporalAdjusters.firstDayOfNextMonth()));
